...
const runnyWhite = "/runny-white-nobg.png";

// Improved CardCurtainReveal components
const CardCurtainReveal = ({ children, className }: { children: ReactNode; className?: string }) => {
  ...
  return (
    ...
    >
      {React.Children.map(children, (child) => {
        ...
        return React.cloneElement(child as ReactElement<{ isHovered?: boolean }>, { isHovered });
      })}
    </div>
  );
};

const CardCurtainRevealBody = ({
  children,
  className,
  isHovered,
}: {
...
const CardCurtainRevealDescription = ({
  children,
  className,
  isHovered,
}: {
...
function paceToSpeed(pace: string | number | null | undefined, distanceKm: number, elapsedTimeMin: number) {
  let paceValue = pace;
...
function isWalk(pace: string | number | null | undefined, distanceKm: number, elapsedTimeMin: number) {
  let paceValue = pace;
...
function formatPace(pace: string | number | null | undefined, distanceKm: number, elapsedTimeMin: number) {
...
function formatTime(minutes: string | number) {
  const mins = Math.floor(parseFloat(String(minutes)));
  const secs = Math.round((parseFloat(String(minutes)) - mins) * 60);
  return `${mins}:${secs.toString().padStart(2, '0')}`;
}

function formatSplitPace(pace: string | number | null | undefined) {
  const paceNum = parseFloat(String(pace ?? ""));
